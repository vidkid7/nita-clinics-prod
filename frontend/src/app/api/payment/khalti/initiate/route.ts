...
export async function POST(req: NextRequest) {
  ...
  const response = await fetch(`${backendUrl}/api/v1/payments/initiate`, {
    ...
    body: JSON.stringify({
      ...
      gateway: 'khalti',
      purpose,
      successUrl: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3002'}/payment/khalti/callback`,
      failureUrl: `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3002'}/payment/khalti/callback`,
    }),
  });
